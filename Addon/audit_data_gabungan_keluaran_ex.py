...
def audit_final_fix():
    ...
    print(f"--- MEMULAI AUDIT PRESISI (MULTI-VALUE SUPPORT) ---")
    ...
    for index, row in df_master.iterrows():
        try:
            if len(row) > IDX_VAL:
                raw_key = str(row.iloc[IDX_KEY]).strip().replace('.','').replace('-','').replace(' ','')
                
                raw_val_str = str(row.iloc[IDX_VAL]).replace(',', '').replace(' ', '')
    ...
    list_files = [f for f in list_files if file_gabungan not in f and "Laporan_Audit" not in f]
    ...
    for file in list_files:
        ...
        try:
            ...
            for idx, row in df_src.iterrows():
                ...
                key_clean = key_ori.strip().replace('.','').replace('-','').replace(' ','')
                
                val_raw_str = str(row.iloc[IDX_VAL]).replace(',', '').replace(' ', '')
                ...
                if found:
                    ...
                    detail_errors.append({
                        ...
                        'Nilai Master (Saran)': matched_val if key_clean in master_dict else 0,
    ...
    if summary_data:
        ...
        with pd.ExcelWriter(file_output_audit, engine='openpyxl') as writer:
            ...
            if detail_errors:
                pd.DataFrame(detail_errors).to_excel(writer, sheet_name='Detail Kesalahan', index=False)
            else:
                pd.DataFrame({'Info': ['Perfect Match']}).to_excel(writer, sheet_name='Detail Kesalahan', index=False)
                    ...
                    try:
                        if cell.value:
                            ...
                            if isinstance(cell.value, (int, float)):
                                ...
                                if any(x in h_val for x in ['nilai', 'ppn', 'selisih', 'total']):
