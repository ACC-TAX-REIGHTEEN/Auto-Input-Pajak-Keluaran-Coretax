    ...
    try:
        ...
        for filename in all_files:
            try:
                ...
                print_debug_csv(f"--> Gagal memproses {filename}: {e}")
...
def jalankan_proses_excel():
    ...
    for urutan, file in enumerate(semua_file):
        try:
            ...
            if len(df.columns) != len(header_utama):
                print(f"--> {file} jumlah kolom beda. Tetap digabung.")
    ...
    if list_data:
        ...
        if semua_file:
            print("--> Gagal proses Excel. Tidak ada data yang berhasil diproses.")
...
def read_file_smart(filepath):
    try:
        ...
        try:
            ...
            return pd.read_csv(filepath, header=3, encoding='latin1', dtype=str)
        except Exception:
            return None
