...
try:
    ...
    col_mapping = {
        ...
        'NPWP/NIK/Nomor Paspor': 'TIN',
        'Faktur Pajak/Dokumen Tertentu/Nota Retur/Nota Pembatalan - Tanggal': 'DocumentDate',
        'PPN (Rupiah)': 'VAT'
    }
...
for i, row in acc_df[acc_df['Match_Status'] == 'Not in Coretax'].iterrows():
    ...
    for c_idx in candidates:
        ...
        if score >= 0.8:
            date_hma = hma_df.at[c_idx, 'Date_Obj']
            day_diff = abs((date_acc - date_hma).days) if not pd.isna(date_acc) and not pd.isna(date_hma) else 999
            
            if (date_acc == date_hma) or (day_diff <= 3):
                acc_df.at[i, 'Match_Status'] = 'Matched'
                acc_df.at[i, 'Match_Index_HMA'] = c_idx
                hma_df.at[c_idx, 'Match_Status'] = 'Matched'
                hma_df.at[c_idx, 'Match_Index_ACC'] = i
...
ws_src_acc = writer.sheets['Data Asli Accurate']
ws_src_hma = writer.sheets['Data Asli Coretax']
ws_src_acc.set_column(0, 20, 20) 
ws_src_hma.set_column(0, 20, 20)
